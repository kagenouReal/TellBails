...
//GK GUNA
  /** Kirim pesan yang otomatis terhapus */
  async sendAutoDeleteMessage(chatId, text, duration) {
  ...
  /** Kirim gambar dengan tombol */
  sendImageButton(chatId, { imageUrl, caption, buttons }) {
    return this.sendPhoto(chatId, imageUrl, {
      caption,
      reply_markup: { inline_keyboard: buttons }
    });
  }
  ...
  sendVideoButton(chatId, { videoUrl, caption, buttons }) {
    return this.sendVideo(chatId, videoUrl, { caption,       reply_markup: { inline_keyboard: buttons } });
  }

  /** Kirim audio dengan tombol */
  sendAudioButtons(chatId, { audioUrl, caption, buttons }) {
    return this.sendAudio(chatId, audioUrl, {
      caption,
      reply_markup: { inline_keyboard: buttons }
    });
  }

  /** Kirim lokasi dengan peta */
  sendLocationMap(chatId, { latitude, longitude, title, address }) {
    return this.sendVenue(chatId, latitude, longitude, title, address);
  }


  /** Kirim polling dengan timer */
  async sendPollTimer(chatId, { question, options, duration }) {
    return new Promise(async (resolve) => {
      let pollMsg = await this.sendPoll(chatId, question, options, { is_anonymous: false });

      setTimeout(() => {
        this.stopPoll(chatId, pollMsg.message_id);
        resolve();
      }, duration * 1000);
    });
  }
  async sendQuickReply(chatId, { text, options }) {
    return this.sendMessage(chatId, text, {
      reply_markup: {
