...
class Telegram extends client_1.default {
   ...
   * @param message pesan yang berisi custom image
   */
  sendCustomImageMessage(chatId, message) {
    return this.callApi('sendPhoto', {
      chat_id: chatId,
      photo: message.imageUrl,  // URL gambar yang akan dikirim
      caption: message.caption, // Caption untuk gambar
    });
  }

  /**
   * Menangani pesan customImageMessage
   * @param message pesan yang memiliki tipe customImageMessage
   */
  handleCustomImageMessage(message) {
    console.log('Received custom image message:', message);
    // Di sini, kamu bisa menambahkan fungsionalitas untuk menangani gambar
  }

  /**
   * Menangani pesan customTextMessage
   * @param message pesan yang memiliki tipe customTextMessage
   */
  handleCustomTextMessage(message) {
  ...
  // Fungsi lainnya seperti getMe, getFile, dll.
  getMe() {
